...
async function saveBookToServer (store, book) {
  ...
  return api('saveBook', book).catch(e => store.commit('addError', e))
}
...
async function getMetadataFromServer (store, id) {
  ...
  let metadata = await api('metadata', id).catch(e => store.commit('addError', e))
  return metadata
}

async function updateFromServer (store) {
  console.log('Updating books list from server')
  let books = await api('books').catch(e => store.commit('addError', e))
  store.state.books.splice(0, store.state.books.length)
  store.state.books.push(...books)

  let imports = await api('imports').catch(e => store.commit('addError', e))
...
async function getUsersFromServer (store) {
  ...
  let users = api('allUsers').catch(e => store.commit('addError', e))
  return users
}

async function addUserToServer (store, user) {
  console.log(`Adding user ${user.username} from server`)
  return api('addUser', user).catch(e => store.commit('addError', e))
}
